...
def norm_logits(logits : torch.Tensor, temperature=0.6, top_k=-1, top_p=0.9) -> torch.Tensor:
    ...
    if logits.dim() == 3:
        ...
        for i in range(token_num):
            logits[:, i,:] = top_k_top_p_filter(logits[:, i,:], top_k=top_k, top_p=top_p)
...
def sample(probs : torch.Tensor, num_samples=1):
    if probs.dim() == 3:
        ...
        idx_next = torch.multinomial(probs_2d, num_samples=1, replacement=True)
        idx_next = idx_next.view(b1, b2)
    else:
